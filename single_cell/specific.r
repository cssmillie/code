specific_markers = function(seur, markers, ident.use=NULL, test.use='mast', data.use='log2', tpm=NULL, n.cores=1, ...){
    ...
    # Find genes that are DE in ALL pairwise comparisons between (ident.use, seur@ident)
	...
	js = rev(names(sort(sapply(js, function(j) length(intersect(markers[ident == i, gene], markers[ident == j, gene]))))))
	    ...
	    # run marker test
	    mi = p.find_markers(seur, ident.1=i, ident.2=j, tpm=tpm, data.use=data, genes.use=genes.use, dir='pos', test.use='mast', do.stats=F, min_fc=0, min_alpha=0, lrt_regex=lrt_regex, ...)
...
reject_markers = function(seur, ident2genes, test.use='mast', data.use='log2', tpm=NULL, n.cores=1, ...){
    ...
    for(i in ident.use){
	    ...
	    # run marker test
	    mi = p.find_markers(seur, ident.1=i, ident.2=j, tpm=tpm, data.use=data, genes.use=genes.use, dir='neg', test.use='mast', do.stats=F, min_fc=1.01, min_alpha=.005, lrt_regex=lrt_regex, ...)
