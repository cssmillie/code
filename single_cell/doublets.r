score_doublets = function(seur, sig, scores=NULL, doublets=NULL, pd=0.25, min_exp=1, min_odds=1, do.fast=FALSE){
    
    # fix input arguments
    if(is.null(doublets)){doublets = rep(FALSE, ncol(seur@data))}
    ...
    # check column names
    sig = sig[names(sig) %in% levels(seur@ident)]
    if(length(sig) == 0){print('error: names(sig) != levels(seur@ident)')}
            
    # score cells
    if(is.null(scores)){
        print('Calculating signature scores')
	scores = score_cells(seur, names=sig, make.names=T)
    }
    i = !doublets
    scores.u = data.frame(aggregate(scores[i,], list(make.names(seur@ident[i])), mean), row.names=1)
    scores.s = data.frame(aggregate(scores[i,], list(make.names(seur@ident[i])), sd), row.names=1)
    
    # parameters for null hypothesis (h0)
    h0.u = scores.u[make.names(as.character(seur@ident)),]
    h0.s = scores.s[make.names(as.character(seur@ident)),]
...
find_doublets = function(seur, sig, scores=NULL, doublets=NULL, pd=0.25, min_exp=1, min_odds=c(4,2,1,1), do.fast=FALSE){

    print('Calculating signature scores')
    sig = sig[names(sig) %in% levels(seur@ident)]
    if(is.null(scores)){scores = score_cells(seur, names=sig)}
    if(is.null(doublets)){doublets = rep(FALSE, ncol(seur@data))}
    scores = scores[,make.names(names(sig))]
    
    res = list()
    for(i in 1:length(min_odds)){
        res[[i]] = score_doublets(seur=seur, sig=sig, scores=scores, doublets=doublets, pd=pd, min_exp=min_exp, min_odds=min_odds[[i]], do.fast=TRUE)
        doublets = res[[i]]$doublets
    }
    return(res)
}

#scores = score_cells(seur, sig=sig.use)
#res = find_doublets(seur, sig.use, scores=scores, pd=.25, min_exp=1.5, min_odds=c(8,7,6,5,4,3,2,1,4))
