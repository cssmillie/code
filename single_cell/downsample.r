...
knn_downsample = function(seur = NULL, counts = NULL, ident = NULL, ngene = NULL, pca.rot = NULL,
    ...
    if(is.null(pca.rot)){
        ...
        pca.rot = pca.rot[colnames(counts), 1:num_pcs]
    }
	...
	if(uniform_rates == FALSE){
	    ...
	    g = nng(pca.rot[cells.use,], k=k1, method=dist.use, use.fnn=FALSE)
	...
	g = nng(pca.rot[cells.use,], k=k2, method=dist.use, use.fnn=FALSE)
