...
sigdb = '/home/unix/csmillie/aviv/db/immgen.%s.db.txt' %(host)
...
# Demultiplex
cmd1 = "cat /home/unix/csmillie/aviv/code/10X/wrap_10x_demux.sh | sed -e 's INDIR %s ' -e 's VERSION %s ' > ./demux.sh" %(args.folder, args.version)
os.system(cmd1)
if check_step('demux'):
    pipeline.append(['sh ./demux.sh'])

# Cell Ranger
cmd2 = "cat /home/unix/csmillie/aviv/code/10X/wrap_10x_run_parallel.sh | sed -e 's BARCODES %s ' -e 's REF %s ' -e 's FQPATH %s ' -e 's VERSION %s ' > ./run10x.sh" %(args.barcodes, ref, fastq_path, args.version)
os.system(cmd2)
...
if check_step('dge'):
    ...
    for sample in samples:
        ...
        cmdi = 'Rscript /home/unix/csmillie/aviv/code/10X/make_counts.r %s %s ./outs/DGE/%s.dge.txt' %(folder, sample, sample)
    ...
    out.close()
    pipeline.append(['Rscript /home/unix/csmillie/aviv/code/merge_dges.r --map all.map.txt --ming 500 --out ./outs/DGE/all.dge.txt; gzip ./outs/DGE/all.dge.txt'])
...
if check_step('tsne'):
    ...
    for sample in samples:
        cmds.append('Rscript /home/unix/csmillie/aviv/code/10X/do_seurat.r --name ./outs/Portal/%s --dge ./outs/DGE/%s.dge.txt.gz --ming 500 --sigdb %s' %(sample, sample, sigdb))
    cmds.append('Rscript /home/unix/csmillie/aviv/code/10X/do_seurat.r --name ./outs/Portal/all --dge ./outs/DGE/all.dge.txt.gz --ming 500 --sigdb %s --out' %(sigdb))
    pipeline.append(cmds)
