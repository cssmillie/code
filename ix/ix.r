...
make_edges = function(data, diff=NULL, weights=NULL, ix=NULL, do.intersect=FALSE, full_complex=FALSE){
    ...
    # Read interactions
    if(is.null(ix)){
        # Load FANTOM5 receptor-ligand interactions
        ix = read.table('PairsLigRec.txt', sep='\t', header=T, stringsAsFactors=F, comment.char='', quote='')
...
find_ix = function(data){
    
    # Find significantly enriched interactions between pairs of cell types
    #
    # The input (data) is a two-column matrix:
    # - column 1 ("ident") = cell type labels
    # - column 2 ("gene") = gene names
    # - note: please make sure you use these column names ("ident" and "gene")
    #
    # The outputs are:
    # - edges = list of cell-cell interactions among all cell types
    # - graph = adjacency matrix (rows/cols = cell types, entries = # receptor-ligand interactions)
    # - shuffled_graphs = adjacency matrices for shuffled graphs (see above)
    # - pvals = p-value matrix (rows/cols = cell types, entries = p-values)
    #
    # Note: this code uses the FANTOM5 database provided in "PairsLigRec.txt"
    # If you would like to modify this, please adjust code in "make_edges"
    
    # Convert data to edgelist
    edges = make_edges(data)
    
    # Construct adjacency matrix from edgelist
    graph = edges2graph(edges)
    
    # Construct 100 shuffled adjacency matrices
    shuffled_graphs = lapply(1:100, function(i) edges2graph(edges, permute=T))
    
    # Get p-values for edges
    pvals = matrix_pvals(graph, shuffled_graphs)
    
    # Return results
    res = list(edges=edges, graph=graph, shuffled_graphs=shuffled_graphs, pvals=pvals)
    return(res)
}
