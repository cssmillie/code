...
# run gblocks
cmd = '~/bin/Gblocks_0.91b/Gblocks %s.temp.aln -b1=%d -b2=%d -b3=%d -b4=%d -b5="%s" -e=".gb"' %(args.aln, args.b1, args.b2, args.b3, args.b4, args.b5)
os.system(cmd)

# decode gblocks output
rmv = tot = 0
out = open('%s.gb' %(args.aln), 'w')
for i, record in enumerate(util.iter_fst('%s.temp.aln.gb' %(args.aln))):
    new = record[0]
    old = gmap[new]
    record[0] = '>%s' %(old)
